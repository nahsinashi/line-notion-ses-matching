 ...
 *
 * 【機能概要】
 * 案件と要員のAIマッチング判定・提案DB登録・管理者通知
 *
 * 【処理フロー】
 * 1. 案件/要員登録時にマッチングキューに追加（遅延実行でタイムアウト対策）
 * 2. キュー処理: 対象ペアごとにAI（Gemini）でスコアリング
 * 3. 70点以上 → 提案DBに「候補」として自動登録
 * 4. 85点以上 → 管理者にLINE通知（承認フローへ）
 *
 * 【閾値】
 * - 提案DB登録: 70点以上
 * - LINE通知: 85点以上
 *
 * 【依存】
 * - 03_AI整形.gs: 整形完了後に scheduleMatching() が呼ばれる
 * - 08_LINE通知.gs: notifyProposalToAdmin()（通知送信）
 */
