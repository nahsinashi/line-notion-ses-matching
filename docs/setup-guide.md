...
> **注意**: このガイドは旧システム（gas-main/gas-admin + Gemini/GPT API）向けです。
> 現行システム（gas-inbox + Claude Code）のセットアップは [改修計画](./改修計画_フォーム省略.md) を参照してください。
> メンバー向けGoogleフォーム（gas-main）のセットアップには引き続き使用できます。
