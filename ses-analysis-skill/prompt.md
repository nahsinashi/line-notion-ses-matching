...
### 3. グラフ生成フェーズ
```
QuickChart.ioでグラフURLを生成:
  - QuickChart.io URL生成方式を使用
  - 円グラフ、棒グラフ、折れ線グラフ等
  - NotionにはURL埋め込みとして追加
↓
進捗を表示:
  ✓ グラフ生成完了（6枚）
```
...
```
Notionレポートを生成:
  1. Markdownレポートを生成（QuickChart.io URLを含む）
  2. 「最新週次/月次レポート」ページを更新（上書き）
  3. 「週次/月次レポート履歴」配下に新規ページ作成
↓
完了メッセージ:
  ✓ 「最新週次レポート」更新完了
