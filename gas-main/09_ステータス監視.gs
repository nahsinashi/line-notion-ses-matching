 ...
 *
 * 【機能概要】
 * 案件/要員/提案DBのステータス変更を検知し、履歴DBに記録
 *
 * 【検知方法】
 * 1. Googleフォーム経由: ステータス変更時に即時記録（01_フォーム送信時の処理スクリプト.gs から呼び出し）
 * 2. Notion直接変更: 定期ポーリングで前回スナップショットと比較し差分を検知
 *
 * 【監視対象DB】
 * - 提案DB（ステータス: 候補/提案中/面談/結果待ち/見送り/辞退/決定）
 * - 要員DB（ステータス）
 * - 案件DB（ステータス）
 *
 * 【トリガー設定】
 * pollStatusChanges() を時間ベーストリガー（例: 10分おき）に設定
 */
